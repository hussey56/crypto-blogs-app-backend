...
// {/*ONly for checking Purpose and usage of cloudinary */}
        //photo decoding
        //*read as buffer
 //   1    const buffer = Buffer.from(photo.replace(/^data:image\/(png|jpg|jpeg);base64,/,''),'base64');

        //*allot a name
//2 const imagePath = `${Date.now()}-${author}.png`;
        
 let response;
try {
    //*save locally
  //3  fs.writeFileSync(`storage/${imagePath}`,buffer)
   //save to cloudinary
...
let newBlog;
 try {
  newBlog  = new Blog({
    ...
    //4 photoPath:`${BACKEND_SERVER_PATH}/storage/${imagePath}`
 });
 await newBlog.save();
 } catch (error) {
    return next(error);  
...
await Blog.updateOne({_id:blogId},
    {title,content,photoPath:response.url}
    );
}else{
    await Blog.updateOne({_id:blogId},
        {title,content} 
